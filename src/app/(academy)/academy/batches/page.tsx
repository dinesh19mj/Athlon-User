  ...
  return (
    <div className="h-[calc(100vh-156px)] md:h-[calc(100vh-64px)] overflow-hidden bg-background text-foreground flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 hide-scrollbar">
        {batches.map((batch) => (
          <div key={batch.id} className="bg-surface/80 backdrop-blur-md border border-foreground/5 rounded-2xl p-5 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <BookOpen className="w-24 h-24" />
            </div>
            
            <h3 className="font-black text-lg text-foreground mb-4 relative z-10">{batch.name}</h3>
            ...
            <div className="mt-5 pt-4 border-t border-foreground/10 flex justify-between items-center relative z-10">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-foreground/40 uppercase tracking-widest">Assigned Coach</span>
                <span className="text-sm font-bold text-foreground">{batch.coach}</span>
              </div>
              <button className="bg-foreground/5 hover:bg-foreground/10 border border-foreground/10 rounded-xl px-4 py-2 text-[10px] font-bold text-foreground uppercase tracking-widest transition-colors">
                Manage
              </button>
            </div>
          </div>
