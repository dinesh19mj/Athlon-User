  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0F1A] text-white font-sans pb-24 overflow-y-auto selection:bg-[#1B9C56] selection:text-black">
      ...
      <main className="w-full max-w-lg mx-auto px-4 flex flex-col gap-6 pt-4">
        ...
        <div className="flex items-center bg-[#121824] p-1 rounded-xl border border-white/10 relative">
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex-1 py-2 text-[11px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all z-10 ${
                  isActive ? 'text-[#0A0F1A]' : 'text-white/50 hover:text-white'
                }`}
              >
                {tab.label}
              </button>
        ...
        <div className="flex flex-col gap-5">
          {tournaments.map((tournament) => (
            <div key={tournament.id} className="bg-[#121824] border border-white/10 hover:border-white/30 rounded-[24px] overflow-hidden transition-colors shadow-lg cursor-pointer group">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <h2 className="text-sm sm:text-base font-black leading-tight text-white group-hover:text-[#1B9C56] transition-colors">
                    {tournament.title}
                  </h2>
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-[#0A0F1A] border border-white/10 flex items-center justify-center">
                    <Trophy className={`w-5 h-5 text-white/50 group-hover:text-[#1B9C56] transition-colors`} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-2 mb-5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] sm:text-xs text-white/70">{tournament.date}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-orange-400" />
                    <span className="text-[10px] sm:text-xs text-white/70 truncate">{tournament.location}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <DollarSign className="w-3.5 h-3.5 text-[#1B9C56]" />
                    <span className="text-[10px] sm:text-xs text-white/70">Prize: <span className="font-bold text-white">{tournament.prize}</span></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Users className="w-3.5 h-3.5 text-purple-400" />
                    <span className="text-[10px] sm:text-xs text-white/70">Fill: <span className="font-bold text-white">{tournament.participants}</span></span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-4">
                  <div className="flex flex-wrap items-center gap-1.5 max-w-[65%]">
                    {tournament.categories.map((cat, idx) => (
                      <span key={idx} className="px-2 py-0.5 rounded border border-white/10 bg-[#0A0F1A] text-[9px] font-medium text-white/50 whitespace-nowrap">
                        {cat}
                      </span>
                    ))}
                  </div>
                  
                  <button className="flex items-center justify-center gap-1 text-[10px] font-bold text-[#1B9C56] hover:text-white transition-colors bg-[#1B9C56]/10 px-3 py-1.5 rounded-lg">
