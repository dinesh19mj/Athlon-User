...
export default function PlayerRankingsPage() {
  ...
  const getRankIcon = (rank: number) => {
    ...
    return <span className="w-5 h-5 flex items-center justify-center font-black text-white/40">{rank}</span>;
  };
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1A] text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <header className="px-4 py-6 flex items-center justify-between shrink-0 relative z-10">
        <button onClick={() => router.push('/player')} className="p-3 -ml-3 text-white/70 hover:text-white transition-colors bg-white/0 hover:bg-white/5 rounded-full">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <div className="flex flex-col items-center">
          <h1 className="text-xl font-black uppercase tracking-widest text-white drop-shadow-md">Global Rankings</h1>
      ...
      <div className="px-4 pb-6 relative z-10 shrink-0">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-[#121824] border border-white/5 rounded-2xl pl-12 pr-4 py-4 text-sm font-bold text-white placeholder-white/30 focus:outline-none focus:border-[#3B82F6]/50 focus:shadow-[0_0_15px_rgba(59,130,246,0.15)] transition-all shadow-xl"
          />
        </div>
        
        {/* Category Chips */}
        <div className="flex gap-2 mt-4 overflow-x-auto hide-scrollbar pb-1 -mx-4 px-4">
          <button className="px-4 py-2 bg-[#3B82F6] text-white rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap shadow-[0_4px_15px_rgba(59,130,246,0.3)]">
            Men's Singles
          </button>
          <button className="px-4 py-2 bg-[#121824] border border-white/5 text-white/60 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-colors">
            Women's Singles
          </button>
          <button className="px-4 py-2 bg-[#121824] border border-white/5 text-white/60 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-colors">
            Mixed Doubles
        ...
        {rankingsData.map((player, i) => (
          <div 
            ...
            className={`flex items-center gap-4 p-4 rounded-2xl transition-all animate-in fade-in slide-in-from-bottom-4 relative overflow-hidden ${
              player.isCurrentUser 
                ...
                : 'bg-[#121824] border border-white/5 hover:bg-white/5'
            }`}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className={`font-extrabold truncate ${player.isCurrentUser ? 'text-[#3B82F6] text-base' : 'text-white text-sm'}`}>
                {player.isCurrentUser ? displayName : player.name}
              </h3>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[10px] font-black uppercase tracking-widest text-white/40">{player.points.toLocaleString()} PTS</span>
              </div>
            </div>
