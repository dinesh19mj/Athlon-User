...
export default function MatchSetupPage() {
  ...
  const CustomSwitch = ({ checked, onChange }: { checked: boolean, onChange: (v: boolean) => void }) => (
    <div 
      ...
      className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors border border-white/5 shadow-inner ${checked ? 'bg-red-500/20 border-red-500/30' : 'bg-[#0A0F1A]'}`}
    >
      <div className={`w-4 h-4 rounded-full transition-transform shadow-lg ${checked ? 'translate-x-6 bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]' : 'translate-x-0 bg-white/40'}`} />
    </div>
  );

  return (
    <div className="h-[100dvh] bg-[#0A0F1A] text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <header className="px-4 py-6 flex items-center justify-between shrink-0 relative z-10">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-xl font-black uppercase tracking-widest text-white drop-shadow-md">Match Setup</h1>
      ...
      <div className="px-4 pb-6 shrink-0 relative z-10">
        <div className="flex bg-[#121824] border border-white/5 p-1.5 rounded-2xl shadow-xl">
          <button 
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider rounded-xl transition-all ${activeTab === 'settings' ? 'bg-red-500 text-white shadow-[0_4px_20px_rgba(239,68,68,0.4)]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
          >
            Settings
          </button>
          <button 
            onClick={() => setActiveTab('team1')}
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider rounded-xl transition-all ${activeTab === 'team1' ? 'bg-red-500 text-white shadow-[0_4px_20px_rgba(239,68,68,0.4)]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
          >
            {isDoubles ? 'Team 1' : 'Player 1'}
          </button>
          <button 
            onClick={() => setActiveTab('team2')}
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider rounded-xl transition-all ${activeTab === 'team2' ? 'bg-red-500 text-white shadow-[0_4px_20px_rgba(239,68,68,0.4)]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
          >
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-3">
              <h2 className="text-[10px] font-black text-white/40 uppercase tracking-widest pl-2">Match Format</h2>
              <div className="grid grid-cols-2 gap-4">
                <button 
                  onClick={() => setCategory('Singles')}
                  className={`flex flex-col items-center justify-center gap-3 p-5 rounded-3xl border transition-all ${!isDoubles ? 'bg-[#121824] border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.15)] scale-[1.02]' : 'bg-[#0A0F1A] border-white/5 hover:border-white/20 hover:bg-white/5'}`}
                >
                  <div className={`p-3 rounded-full ${!isDoubles ? 'bg-red-500/20 text-red-500' : 'bg-white/5 text-white/40'}`}>
                    <User className="w-6 h-6" />
                  </div>
                  <span className={`text-sm font-bold uppercase tracking-wide ${!isDoubles ? 'text-white' : 'text-white/40'}`}>Singles</span>
                </button>
                <button 
                ...
                >
                  ...
                  <span className={`text-sm font-bold uppercase tracking-wide ${isDoubles ? 'text-white' : 'text-white/40'}`}>Doubles</span>
                </button>
              </div>
            </div>
            ...
            {/* Rules & Configuration */}
            <div className="space-y-3">
              <h2 className="text-[10px] font-black text-white/40 uppercase tracking-widest pl-2">Rules & Scoring</h2>
              ...
              <div className="bg-[#121824] border border-white/5 rounded-3xl overflow-hidden shadow-xl">
                ...
                <div className="flex items-center justify-between p-5 border-b border-white/5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-[#0A0F1A] rounded-xl border border-white/5">
                      <ListOrdered className="w-5 h-5 text-white/70" />
                    </div>
                    <span className="text-sm font-bold text-white/90">Max games (sets)</span>
                  </div>
                  <div className="relative">
                    ...
                    <div className="bg-[#0A0F1A] border border-white/10 rounded-xl px-4 py-2 flex items-center gap-4 pointer-events-none shadow-inner">
                ...
                <div className="flex items-center justify-between p-5 border-b border-white/5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-white/90">Points per game</span>
                  </div>
                  <div className="relative">
                    <select 
                      value={pointBreak}
                ...
                <div className="flex items-center justify-between p-5 border-b border-white/5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-white/90">Use Deuce</span>
                  </div>
                ...
                <div className="flex items-center justify-between p-5 border-b border-white/5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-white/90">Use Seeding</span>
                  </div>
                ...
                <div className="flex items-center justify-between p-5 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-white/90">Use Countries</span>
                  </div>
        ...
        {activeTab === 'team1' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
            <div className="bg-[#121824] border border-white/5 rounded-3xl p-6 shadow-xl">
              <h2 className="text-lg font-black uppercase tracking-wide mb-6 text-white drop-shadow-md">
                {isDoubles ? 'Team 1 Roster' : 'Player 1 Details'}
              </h2>
              
              <div className="space-y-5">
                <div>
                  <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-2 ml-1">Player 1 Name</label>
                  <input 
                    type="text" 
                    value={teamA[0]}
                    onChange={(e) => setTeamA([e.target.value, teamA[1]])}
                    placeholder="Enter full name"
                    className="w-full bg-[#0A0F1A] border border-white/10 rounded-2xl px-5 py-4 text-white font-bold focus:outline-none focus:border-red-500 focus:shadow-[0_0_15px_rgba(239,68,68,0.2)] transition-all placeholder-white/20"
                  />
                </div>
                {isDoubles && (
                  <div className="animate-in fade-in slide-in-from-top-4 duration-300">
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-2 ml-1">Player 2 Name</label>
                    <input 
                      type="text" 
                      value={teamA[1]}
      ...
      <div className="fixed bottom-20 md:bottom-0 left-0 right-0 p-4 md:p-6 bg-gradient-to-t from-[#0A0F1A] via-[#0A0F1A]/95 to-[#0A0F1A]/0 backdrop-blur-sm pt-12 z-40">
        <button 
          onClick={handleNext}
          className="w-full bg-red-500 text-white font-black uppercase tracking-widest py-4 rounded-2xl text-sm flex items-center justify-center gap-2 hover:bg-red-400 active:scale-[0.98] transition-all shadow-[0_10px_40px_rgba(239,68,68,0.3)] border border-red-400/50"
        >
