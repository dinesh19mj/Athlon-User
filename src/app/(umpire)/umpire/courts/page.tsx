    ...
    <div className="min-h-screen bg-background text-foreground font-sans flex flex-col relative overflow-hidden">
      ...
      <header className="px-4 py-6 flex items-center justify-between shrink-0 relative z-10">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-xl font-black uppercase tracking-widest text-foreground drop-shadow-md">Venue Courts</h1>
      ...
      <div className="px-4 pb-6 grid grid-cols-3 gap-3 relative z-10 shrink-0">
        <div className="bg-surface border border-foreground/5 rounded-2xl p-3 shadow-lg flex flex-col items-center justify-center">
          <span className="text-xl font-black text-foreground">4</span>
          <span className="text-[9px] font-bold uppercase tracking-widest text-foreground/40 mt-1">Total</span>
        </div>
        <div className="bg-surface border border-green-500/20 rounded-2xl p-3 shadow-lg flex flex-col items-center justify-center">
        ...
        <div className="bg-surface border border-blue-500/20 rounded-2xl p-3 shadow-lg flex flex-col items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 relative z-10 hide-scrollbar space-y-4">
        {venueCourts.map((court, i) => (
          <div 
            key={court.id}
            className={`bg-surface border border-foreground/5 rounded-3xl p-5 shadow-xl animate-in fade-in slide-in-from-bottom-4 relative overflow-hidden`}
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-foreground/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-foreground tracking-wide uppercase">{court.name}</h3>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 mt-0.5">{court.surface}</p>
                </div>
              </div>
            ...
            {court.currentMatch ? (
              <div className="bg-background p-4 rounded-2xl border border-foreground/5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-black uppercase tracking-widest text-foreground/50">{court.currentMatch.category}</span>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 truncate w-[40%]">
                    ...
                    <span className="text-sm font-bold text-foreground truncate">{court.currentMatch.teamA}</span>
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-foreground/20 shrink-0">VS</span>
                  <div className="flex items-center justify-end gap-2 truncate w-[40%] text-right">
                    <span className="text-sm font-bold text-foreground truncate">{court.currentMatch.teamB}</span>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-background/50 border border-foreground/5 border-dashed p-6 rounded-2xl flex flex-col items-center justify-center">
            ...
            {court.status === 'AVAILABLE' && (
              <button 
                ...
                className="w-full mt-4 py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-foreground text-xs font-black uppercase tracking-widest transition-colors shadow-[0_5px_20px_rgba(147,51,234,0.3)]"
              >
